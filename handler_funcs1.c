...
{
    stack_t *temp_node = *stack, *new_node = NULL;
    int num;
    ...
    temp_node = get_dnodeint_at_index(*stack, 0);
    num = temp_node->n;
    delete_dnodeint_at_index(stack, 0);
    new_node = insert_dnodeint_at_index(stack, 1, num);
    if (!new_node)
    {
...
void add_handler(stack_t **stack, unsigned int line_number)
{
    ...
    delete_dnodeint_at_index(stack, 0);
    delete_dnodeint_at_index(stack, 0);
    result_node = add_dnodeint(stack, sum);
    if (!result_node)
    {
        dprintf(STDERR_FILENO, MALLOC_FAIL);
        free_all(1);
